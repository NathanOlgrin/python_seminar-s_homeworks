...
for i in range(n):
    list_n.append(int(input('Введите количество ягод на кусте ')))

max = 0
for i in range(n):
    if n - i == 1:
        if max < (list_n[i-1]+list_n[i]+list_n[0]):
            max = list_n[i-1]+list_n[i]+list_n[0]
    else: 
        if max < (list_n[i-1]+list_n[i]+list_n[i+1]):
            max = list_n[i-1]+list_n[i]+list_n[i+1]
